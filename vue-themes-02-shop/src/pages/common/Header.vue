...
<template>
    <nav class="navbar navbar-expand-sm top-0" :class="{
        ...
        <div :class="props.transparent || props.light || props.dark
            ...
            <div class="collapse navbar-collapse w-100 pt-3 pb-2 py-lg-0" id="navigation">
                <ul class="navbar-nav navbar-nav-hover ms-auto">
                    ...
                    <li class="nav-item dropdown dropdown-hover mx-2">
                        <RouterLink to="/shop" class="nav-link ps-2 d-flex cursor-pointer align-items-center"
                            :class="getTextColor()" id="dropdownMenuPages" data-bs-toggle="dropdown"
                            aria-expanded="false">
                            <i class="material-icons opacity-6 me-2 text-md" :class="getTextColor()">category</i>
                            전체
                        </RouterLink>
                    </li>
                    <li class="nav-item dropdown dropdown-hover mx-2">
                        <RouterLink to="/shop?category=키보드" class="nav-link ps-2 d-flex cursor-pointer align-items-center"
                            :class="getTextColor()" id="dropdownMenuPages" data-bs-toggle="dropdown"
                            aria-expanded="false">
                            <i class="material-icons opacity-6 me-2 text-md" :class="getTextColor()">keyboard</i>
                            키보드
                        </RouterLink>
                    </li>
                    <li class="nav-item dropdown dropdown-hover mx-2">
                        <RouterLink to="/shop?category=마우스" class="nav-link ps-2 d-flex cursor-pointer align-items-center"
                            :class="getTextColor()" id="dropdownMenuPages" data-bs-toggle="dropdown"
                            aria-expanded="false">
                            <i class="material-icons opacity-6 me-2 text-md" :class="getTextColor()">mouse</i>
                            마우스
                        </RouterLink>
                    </li>
                </ul>
                <ul class="navbar-nav d-lg-block d-none">
                    <li class="nav-item">
                        <a :href="action.route" class="btn btn-sm mb-0" :class="action.color"
                            onclick="smoothToPricing('pricing-soft-ui')">로그인</a>
                    </li>
                </ul>
            </div>
        </div>
